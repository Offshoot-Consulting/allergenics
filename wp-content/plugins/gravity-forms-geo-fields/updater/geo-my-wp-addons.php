	...
	public function output() {
		...
		*/
		...
		<div class="wrap">
			...
			$output .=          '<div style="display: inline-block;"><a href="http://www.geomywp.com" target="_blank">'.__( 'Developed by Eyal Fitoussi - please take a moment to support my work' ,'GGF').'</a></div>';
			$output .=          '<div>';
			$output .=          	'<a href="http://wordpress.org/plugins/geo-my-wp/" title="Rate GEO my WP" target="_blank"><img src="'.GGF_URL .'/updater/assets/images/star-icon.png" style="max-height:23px;" /></a>';
			...
			<h2 class="gmw-wrap-top-h2">
				<?php _e('GEO my WP Add-ons','GGF'); ?>
			</h2>
		
			<div class="clear"></div>
		
			<div style="float: left; margin-bottom: 10px;">
				<div style="border-left: 4px solid red; background: #FDEFEF;"
					class="gmw-addons-page-top-buttons">
					<?php _e('Add-on Uninstalled / Deactivated', 'GGF'); ?>
				</div>
				<div style="background: #E8F2F5; border-left: 4px solid #2ea2cc;"
					class="gmw-addons-page-top-buttons">
					<?php _e('Add-on Activated', 'GGF'); ?>
				</div>
				<div style="border-left: 4px solid #37C42A; background: #E9F5E8;"
					class="gmw-addons-page-top-buttons">
					<?php _e('License Key Activated', 'GGF' ); ?>
				</div>
			</div>
			<br />
			...
			<form method="post" action="options.php">
				...
				<ul class="widefat fixed">
					...
					<li
						class="gmw-single-addon-wrapper <?php echo $addon['name']; ?> first">
						...
						<div class="gmw-addon-top-wrapper">
							...
							<h2 class="gmw-addon-title">
								...
								<span style="float: right;"> <?php if (isset($addon['version'])) echo $addon['version']; ?>
								</span>
							</h2>
						...
						<div
							class="gmw-addon-license-wrapper gmw-license-invalid gmw-addon-deactivate">
							...
							<a
								href="<?php echo get_admin_url();?>/plugin-install.php?tab=search&s=geo+my+wp&plugin-search-input=Search+Plugins"
								class="button-secondary button-primary gmw-addon-activation-btn"><?php _e('Install Now','GGF'); ?>
							</a>
							...
							<button type="submit"
								...
								name="gmw_addon_activated" <?php echo $disabled; ?>
								value="<?php echo $addon['name']; ?>">
								<?php _e('Activate Add-on','GGF'); ?>
							</button>
							<?php } ?>
		
						</div> <!-- when add-on requires license key and key entered and saved in database -->
						
						<?php } elseif (isset($addon['license']) && $addon['license'] == true) { ?>
		
						<!-- if license is valid --> <?php if (isset($this->statuses[$addon['name']]) && $this->statuses[$addon['name']] !== false && $this->statuses[$addon['name']] == 'valid' && isset($this->licenses[$addon['name']]) && !empty($this->licenses[$addon['name']] ) ) { ?>
		
						<?php wp_nonce_field($addon['name'], $addon['name']); ?>
		
						<div
							class="gmw-addon-license-wrapper gmw-license-valid gmw-addon-activate">
		
							<input class="gmw_license_keys" disabled="disabled" type="text"
								class="regular-text"
								style="width: 100% !important; height: 28px !important; padding: 0px 5px 0px !important; max-width: 268px !important;"
								value="<?php if (isset($this->licenses[$addon['name']]) && !empty($this->licenses[$addon['name']])) echo $this->licenses[$addon['name']]; ?>" />
		
							<input type="hidden"
								name="gmw_license_keys[<?php echo $addon['name']; ?>]"
								value="<?php if (isset($this->licenses[$addon['name']]) && !empty($this->licenses[$addon['name']])) echo $this->licenses[$addon['name']]; ?>" />
		
							<!-- show deactivate license button -->
							<button type="submit"
								class="button-secondary activate-license-btn gmw-addon-activation-btn"
								style="float: right !important; margin-top: 1px !important; padding: 0 9px !important; color: rgb(182, 42, 42) !important; height: 27px !important;"
								name="gmw_license_key_deactivate"
								title="<?php _e('Deactivate License Key', 'GGF'); ?>"
								value="<?php echo $addon['name']; ?>">&#10007;</button>
						...
						</div> <!-- if status invalid --> <?php } else { ?>
		
						<div
							class="gmw-addon-license-wrapper gmw-license-invalid gmw-addon-activate">
							...
							if ( isset( $this->licenses[$addon['name']] ) && !empty( $this->licenses[$addon['name']] ) ) {
								...
								if ( isset( $this->statuses[$addon['name']] ) && ( $this->statuses[$addon['name']] == 'expired' || $this->statuses[$addon['name']] == 'no_activations_left' || $this->statuses[$addon['name']] == 'missing' ) ) {
									if ( $this->statuses[$addon['name']] == 'expired' ) {
										$message = __( 'Your license key has expired. Please renew your key in order to keep getting its updates.', 'GGF' );
									} elseif ( $this->statuses[$addon['name']] == 'no_activations_left' ) {
										$message = __( 'Your license key has no activations left.', 'GGF' );
									} else {
										$message = __( 'Something is wrong with the key you entered. Please check your key and try again.', 'GGF' );
									}
									?>
							<div
								class="gmw-addon-license-wrapper gmw-license-invalid gmw-license-error-wrapper">
		
								<span style="float: left; width: 268px;"><?php echo $message; ?> </span>
								<input type="button"
									class="button-secondary activate-license-btn gmw-addon-activation-btn gmw-remove-license-warning"
									style="float: right !important; margin-top: 4px !important; padding: 0 8px !important; color: rgb(182, 42, 42) !important; height: 27px !important;"
									value="&#10007;" />
							</div>
							<?php
								}
							...
							<input class="gmw_license_keys gmw-addon-short-input"
								name="gmw_license_keys[<?php echo $addon['name']; ?>]" type="text"
								class="regular-text"
								placeholder="<?php _e('Enter your license key', 'GGF'); ?>"
								value="<?php if (isset($this->licenses[$addon['name']]) && !empty($this->licenses[$addon['name']])) echo $this->licenses[$addon['name']]; ?>" />
							...
							<button type="submit"
								...
								title="<?php _e('Activate License Key', 'GGF'); ?>"
								style="float: right !important; margin-top: 1px !important; padding: 0 8px !important; color: green !important; height: 27px !important;"
								value="<?php echo $addon['name']; ?>">&#10003;</button>
						...
						<div class="gmw-addon-license-wrapper gmw-addon-activate">
							...
							<input class="gmw_license_keys"
								name="gmw_license_keys[<?php echo $addon['name']; ?>]" type="text"
								class="regular-text" disabled="disabled"
								placeholder="<?php _e('No license key required', 'GGF'); ?>"
								style="width: 100% !important; height: 28px !important; padding: 3px 8px 0px !important;"
								value="<?php if (isset($this->licenses[$addon['name']]) && !empty($this->licenses[$addon['name']])) echo $this->licenses[$addon['name']]; ?>" />
		
							<button type="submit"
								class="button-secondary gmw-addon-activation-btn"
								style="opacity: 0;" name="gmw_addon_deactivated"
								value="<?php echo $addon['name']; ?>">
								<?php _e('Deactivate Add-on', 'GGF'); ?>
							</button>
						</div> <?php } ?>
