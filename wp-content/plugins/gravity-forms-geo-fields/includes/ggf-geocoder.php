...
function ggf_geocoder( $address, $force_refresh = false ) {
	...
	if ( $force_refresh || $coordinates === false ) {
		...
		$url        = add_query_arg( $args, '//maps.googleapis.com/maps/api/geocode/json' );
		...
		if ( $response['response']['code'] == 200 ) {
			...
			if ( $data->status === 'OK' ) {
				...
				$location['state'] = esc_attr($ac->short_name);
				...
				$location['country'] = esc_attr($ac->short_name);
